...
model.compile(optimizer='adam', loss='binary_crossentropy', metrics=[mean_iou, 'mae', 'acc'])#mean_absolute_error
model.summary()
...
checkpointer = ModelCheckpoint('model-CTseg-7sigVal-2mse.h5', verbose=1, save_best_only=True)
results = model.fit(X_train, Y_train, validation_data=(X_val, Y_val), batch_size=32, epochs=80, 
                    callbacks=[earlystopper, checkpointer])# validation_split=0.125
...
# 绘制训练 & 验证的准确率值
# results.history
plt.plot(results.history['acc'][1:])
...
# model = load_model('model-CTseg-4added-2.h5', custom_objects={'mean_iou': mean_iou})
...
for i in range(len(preds_test)):
    preds_test_upsampled.append(np.squeeze(preds_test[i]))
